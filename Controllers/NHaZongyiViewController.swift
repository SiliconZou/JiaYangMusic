...
import UIKit
import CoreData
class NHaZongyiViewController: UIViewController,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    
    @IBOutlet weak var collectionView: UICollectionView!
    var dataArr = [ContentModel]()
    var nameArr = [String]()
    //http://mapiv2.yinyuetai.com/navigation/list.json?area=KR&type=KR_CUT&offset=0&size=20
    var page = 0
    ...
    func loadData(con:Int) {
        let url = String.init(format: "http://mapiv2.yinyuetai.com/navigation/list.json?area=KR&type=KR_CUT&offset=%ld&size=20", con)
