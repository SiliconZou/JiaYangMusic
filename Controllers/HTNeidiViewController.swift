...
import UIKit
import CoreData
class HTNeidiViewController: UIViewController,UICollectionViewDataSource,UICollectionViewDelegateFlowLayout {
    ...
    func loadData(con:Int) {
        let url = String.init(format: "http://mapiv2.yinyuetai.com/component/list.json?area=HOT_ML&offset=%ld&size=20", con)
        BaseRequest.getWithURK(url: url, para: nil) { (data, error) in
                ...
                if dic["data"] == nil {
                    ...
                    let ac1 = UIAlertAction.init(title: "是", style: .default, handler: nil)
                        acon.addAction(ac1)
                    ...
                    if error == nil {
                        ...
                        for obj in dicArr {
                            ...
                            model.title = obj["desc"] as? String
